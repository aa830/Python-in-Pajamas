...
def display_toc_menu(stdscr):
    """Displays the Table of Contents menu."""
    ...
    # Automatically return Course 1 if no navigation occurred
    return selected_course + 1 if selected_course >= 0 else 1
...
def main1(stdscr, course_number):
    ...
    ascii_art1_content = ascii_art1.splitlines()
    ascii_art2_content = ascii_art2.splitlines()
    ...
    while key != ord('q'):
        ...
        if isinstance(pages_course[current_page], list):  # If it's ASCII art
            for ascii_line in pages_course[current_page]:
                if i < max_y - 1:
        ...
        stdscr.addstr(max_y - 1, 1, "Press 'Right Arrow' to continue, 'q' to quit.")
        ...
        if key == curses.KEY_RIGHT:
            if current_page < len(pages_course) - 1:
                ...
                stdscr.addstr(max_y - 2, 1, "You've reached the end of this course! Press 'q' to exit or select another course.")
                stdscr.refresh()
                stdscr.getch()  # Wait for user to acknowledge

def main(stdscr):
    """Main function to run the program."""
    course_number = display_toc_menu(stdscr)  # Get the selected course number
    main1(stdscr, course_number)  # Display course content
