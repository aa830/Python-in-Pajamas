...
def main1(stdscr):
    ...
    #Content of each page for user's learning
    pagesCourse1 = [
        ...
        "Page 2:\n\nContinuing....\nThat illustration shows the Central Processing Unit (CPU) telling the display to light up! By saying, 'Hello! You seem bright today!'\nObviously, it's not as simple as giving the screen a compliment, but that is essentially how it works. The CPU is like the president, telling all the other parts what to do, and it uses binary to tell them what to do.\n\n To make it simple, lets say that you pressed 'a'. Think about it like the brain, when you touch something hot, your hand will quickly move back, right? It is essentially the same thing!\n When you press the 'a' key, the Mr A Key quickly sends a message using Binary to the CPU (aka the Brain), the CPU decides what to do. Lets say that in this case, the CPU decides to display the letter 'a' on the screen.\nThe CPU sends a Binary message to Mr. Screen to display 'a'.\n\n Now you know how what happens when you type into YouTube!",
    ...
    while key != ord('q'):
        ...
        text = pagesCourse1[current_page]
        ...
        if key in [curses.KEY_RIGHT, ord('n')]:
            if current_page < len(pagesCourse1) - 1:
                current_page += 1
